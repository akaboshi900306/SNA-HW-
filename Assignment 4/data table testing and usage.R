...
# start with base r
system.time(read.csv("C:/Users/akabo/Downloads/social network analysis/reddit_data.csv", header = TRUE))
# almost 90 seconds

# from Hadley Wicham's readr
library(readr)
system.time(read_csv("reddit_data.csv", col_names = TRUE))
# 8 seconds

# data table
library(data.table)
system.time(fread("reddit_data.csv", header = TRUE))
# 3 seconds


# okay, so now let's load this into r and peform a few grouping operations
red = read.csv("reddit_data.csv", header = TRUE)
...
# data table
red2 = fread("C:/Users/akabo/Downloads/social network analysis/reddit_data.csv", header = TRUE)
...
merge <- merge(red2, rdt[,-3], all.x = TRUE)
