...
all_edges = fread("C:/Users/akabo/Downloads/social network analysis/Assignment 1/classroom_social_and_task_network.csv", head = TRUE)
...
social_indegree <- degree(network_social, v = V(network_social), mode = c("in"),
                          loops = TRUE, normalized = FALSE)
social_closeness <- closeness(network_social, vids = V(network_social),
...
# ???for loop ?????? 
a= cbind(netstats_task$deg_in,netstats_task$deg_out,netstats_task$close,netstats_task$betw,netstats_task$prank)
b= cbind(netstats_social$deg_in,netstats_social$deg_out,netstats_social$close,netstats_social$betw,netstats_social$prank)
...
# set the color of the edges using the edge attribute
...
# this is similar to calculating triadic closure, but the triadic closure function in igraph would also count weak ties when looking for a potentially open triple, and we want only potential triples in which at least two of the ties are strong

# checking for a strong tie existing both directions on the edgelist and get the distinct edges
# using data.table avoids having to define separate column names during rbind 
...
# both results suggest moderately alignment to triadic closure
...
for(i in seq_len(nrow(network_matrix) - 2)){
	...
	network_pathcheck = network_pathcheck + network_matrix_power # counts the total number of paths between two nodes of any length, up to the number of nodes in the network minus 1
}
